/**
 * CLI for interacting with midnight-level1 contract
 */
...
// the same private state. The eligibility contract has no witnesses (empty state).
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║                   midnight-level1 CLI                           ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    while (running) {
      console.log('─── Menu ───────────────────────────────────────────────────────');
      console.log('  1. Prove eligibility privately');
      console.log('  2. Read public eligibility result');
      ...
      switch (choice.trim()) {
        case '1': {
          const input = await rl.question('  Enter your private value: ');
          const secretValue = Number(input.trim());

          if (!Number.isInteger(secretValue) || secretValue < 0 || secretValue > 65535) {
            console.log('\n  ❌ Enter an integer between 0 and 65535.\n');
            break;
          }

          console.log('\n  🔐 Proving privately...');
          console.log('  Your input will NOT be stored on the public ledger.');

          try {
            const tx = await deployed.callTx.proveEligibility(BigInt(secretValue));
            console.log('\n  ✅ Proof submitted successfully.');
            console.log(`  Public result: ${secretValue >= 18 ? 'eligible' : 'not eligible'}`);
        ...
        case '2': {
          console.log('\n  Reading public eligibility state...');
          try {
            const contractState = await providers.publicDataProvider.queryContractState(deployment.address);
            if (contractState) {
              const ledgerState = Eligibility.ledger(contractState.data);
              console.log(`\n  📋 Public eligibility: ${ledgerState.eligible}\n`);
              console.log('  🔒 The private input is not part of the public ledger.\n');
            } else {
