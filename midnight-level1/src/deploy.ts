/**
 * Deploy midnight-level1 contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
// eligibility contract has no witnesses, so its private state is empty ({}).
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy midnight-level1 to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState (empty here — the eligibility contract has no
      // witnesses). args is the contract constructor's arguments: empty for
      // eligibility's no-arg constructor. (Statically-typed contracts can omit
